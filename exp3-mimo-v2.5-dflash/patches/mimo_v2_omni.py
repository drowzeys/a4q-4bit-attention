    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, loaded_weight in weights:
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                # EXP3 PATCH: tolerate structural skew between this NVFP4
                # checkpoint's vision tower and the vLLM module (e.g.
                # merger.mlp.*.bias). Text-only serving; vision unused.
...
class MiMoV2OmniForCausalLM(nn.Module, SupportsMultiModal, SupportsPP, SupportsQuant):
    # EXP3 PATCH (per eugr fix-mimo-v2-vllm, June): the Omni class is the
    # SupportsQuant top-level that mutates ModelOptMixedPrecisionConfig.
    # Without a packed_modules_mapping here, fused gate_up_proj modules do
    # not resolve the checkpoint's separate gate_proj/up_proj quantization
    # entries and are treated as UNQUANTIZED, silently corrupting the text
    # path (dense MLP + every shared expert). qkv_proj maps to itself since
    # the checkpoint stores fused qkv entries directly.
