...
def _shard_fp8_qkv_proj(
    ...
    # EXP3 PATCH (GB10, lukealonso/MiMo-V2.5-NVFP4): this checkpoint stores
    # per-output-row group scales (s_full.shape[0] == w_full.shape[0], e.g.
    # scale [13568, 128] for weight [13568, 4096] = 32-wide column groups),
    # not 128x128 block scales. Per-row scales never straddle the Q/K/V
    # boundaries, so the de-interleave is an exact row permutation applied
    # identically to weights and scales -- no dequant/requant needed.
    if s_full.shape[0] == w_full.shape[0]:
        idx_q: list[int] = []
        idx_k: list[int] = []
        idx_v: list[int] = []
        for g_idx in range(
            ...
            base = g_idx * rows_per_group
            idx_q.extend(range(base, base + q_rows_per_group))
            idx_k.extend(
                range(base + q_rows_per_group, base + q_rows_per_group + k_rows_per_group)
            )
            idx_v.extend(
                range(base + q_rows_per_group + k_rows_per_group, base + rows_per_group)
            )
        idx = torch.tensor(idx_q + idx_k + idx_v, device=w_full.device)
        return w_full.index_select(0, idx), s_full.index_select(0, idx)
...
class MiMoV2Model(nn.Module):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, loaded_weight in weights:
            ...
            if self._try_load_fp8_qkv_proj(
                ...
                continue
            # EXP3 PATCH: MXFP8 group scales are serialized as
            # "*.weight_scale_inv" (uint8 E8M0) but the MXFP8 linear method
            # registers the param as "*.weight_scale". Rename so the dense
            # MLP (layer 0) scales load instead of being silently skipped.
            if name.endswith(".weight_scale_inv") and loaded_weight.dtype == torch.uint8:
    ...
    def _try_load_fp8_qkv_proj(
        ...
        """
        # EXP3 PATCH (per eugr fix-mimo-v2-vllm, validated June TP=4/TP=2):
        # lukealonso/MiMo-V2.5-NVFP4 stores fused qkv_proj as canonical
        # [Q_all][K_all][V_all] ("qkv-deinterleaved"), NOT the per-KV-head
        # interleaved Pro layout this helper assumes. Any group-wise reshard
        # here semantically corrupts K/V rows. Fall through and let
        # QKVParallelLinear.weight_loader do Q/K/V-aware TP slicing of both
        # the FP8 weight and the row-aligned MXFP8 scale.
        return False
        is_weight = (
        ...
        for kind, tensor in sharded.items():
            ...
            # EXP3 PATCH: modelopt_mixed MXFP8 layers register the group scale
            # as "weight_scale" (E8M0 uint8), while the checkpoint names the
            # fused-QKV tensor "weight_scale_inv". Map to whichever exists.
            if param_name not in params_dict and kind == "weight_scale_inv":
                param_name = f"{prefix}.weight_scale"
            if param_name not in params_dict:
                cand = sorted(k for k in params_dict if k.startswith(prefix))
                raise KeyError(
                    f"EXP3 DEBUG: no {kind} param for {prefix}; "
                    f"available params on this layer: {cand}; "
                    f"sharded tensor shape={tuple(tensor.shape)} dtype={tensor.dtype}"
                )
