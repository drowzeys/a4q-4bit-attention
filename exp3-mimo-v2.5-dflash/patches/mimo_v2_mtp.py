...
class MiMoV2MTP(nn.Module):
    # EXP3 PATCH (per eugr fix-mimo-v2-vllm): without packed_modules_mapping,
    # ModelOptMixedPrecisionConfig cannot unfuse the MTP's gate_up_proj to the
    # checkpoint's separate gate_proj/up_proj MXFP8 entries, so the MTP dense
    # MLP silently loads UNQUANTIZED -> garbage draft -> acceptance ~0.
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        # EXP3 PATCH: keep MXFP8 weight_scale / weight_scale_inv aliases.
        ...
        for name, loaded_weight in weights:
            ...
            # EXP3 PATCH (per eugr fix-mimo-v2-vllm): MXFP8 group scales are
            # serialized as "*.weight_scale_inv" (uint8) but the MXFP8 linear
            # method registers the param as "*.weight_scale". Rename so MTP
            # dense-MLP + qkv scales actually load (else draft is garbage and
            # acceptance collapses to ~0).
            if name.endswith(".weight_scale_inv") and loaded_weight.dtype == torch.uint8:
                name = name.replace(".weight_scale_inv", ".weight_scale")

            # EXP3 PATCH (per eugr fix-mimo-v2-vllm): MiMo-V2.5-NVFP4 stores
            # fused qkv_proj as canonical [Q_all][K_all][V_all], NOT the
            # per-KV-head-interleaved Pro layout. A blind chunk(tp_size) is
            # shape-correct but corrupts K/V rows and their row-aligned MXFP8
            # scales. Use QKVParallelLinear.weight_loader for Q/K/V-aware TP
            # slicing of both the fp8 weight and the scale.
            if "qkv_proj" in name:
